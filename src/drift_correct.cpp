...
DriftCorrector::DriftCorrector(float timeDelta, float cornerFrequency) noexcept {
  if (!std::isfinite(cornerFrequency) || cornerFrequency <= 0.0f) {
    errors::doFatalError("invalid corner frequency passed into drift corrector",
                         errors::SOFTWARE_INITIALIZATION_FAILED);
  }
  if (!std::isfinite(timeDelta) || timeDelta <= 0.0f) {
    errors::doFatalError("invalid time delta passed into drift corrector", errors::SOFTWARE_INITIALIZATION_FAILED);
  }
  ...
  if (!std::isfinite(m_alpha) || !std::isnormal(m_alpha)) {
    errors::doFatalError("corner frequency given to drift corrector is too great or too small",
                         errors::SOFTWARE_INITIALIZATION_FAILED);
  }
}
